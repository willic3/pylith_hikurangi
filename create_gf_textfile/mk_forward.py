...
class MkForward(Application):
    ...
    ## @li \b response_strain_files PyLith strain response HDF5 file(s).
    ...
    responseStrainFiles = pythia.pyre.inventory.list("response_strain_files",
                                                     default=["resp_fault_obs1.h5", "resp_fault_obs2.h5"])
    responseStrainFiles.meta['tip'] = "PyLith strain response HDF5 file(s)."
    ...
    def __init__(self, name="mk_forward"):
    ...
    def _readGF(self):
        ...
        impVals = impulses['vertex_fields/slip'][:,:,0]
        ...
        # Assumption is a single strain cell per file.
        numStrainRespFiles = len(self.responseStrainFiles)
        respDilCoords = numpy.zeros((numStrainRespFiles,2), dtype=numpy.float64)
        respDilVals = numpy.zeros((numImpulses, numStrainRespFiles), dtype=numpy.float64)
        for strainRespNum in range(numStrainRespFiles):
            strainRespFile = self.responseStrainFiles[strainRespNum]
            if (strainRespFile != ''):
                ...
                strainValsSort = strainVals[impSort,:,:]
                respStrain.close()
                respDilCoords[strainRespNum,:] = cellCenter
                respDilVals[:,strainRespNum] = (strainValsSort[:,0,0] + strainValsSort[:,0,1])
